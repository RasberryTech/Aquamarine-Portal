...
const language = require('./static/languages/en.json');
...
app.use(function(req, res, next) {
    res.setHeader(
        ...
        "1|https,res.cloudinary.com,,2|1|https,res.cloudinary.com,/dpkpng0q9/image/upload/v1712519157/paintings/,6"
    );
    next();
});
...
const port = Number(
    process.env.PORT || config_http.port || 8081
);
...
try {
    environment = JSON.parse(
        process.env.ENVIRONMENT || '{"ENV_NAME":"development"}'
    );
} catch {
    environment = {
...
app.listen(port, () => {
    console.log(
        "[INFO] Current Environment: %s. Listening on port %d",
