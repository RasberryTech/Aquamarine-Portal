...
const strictMode =
    String(process.env.AQUAMARINE_STRICT_MODE || 'false').toLowerCase() === 'true';
...
async function auth(req, res, next) {
    if (
        ...
        (req.path.includes('v1') && req.path.includes('users'))
    ) {
        return next();
    }

    // Allow local browser development without Wii U authentication headers.
    if (
    ...
    // Account creation and public people pages remain reachable without an account.
    if (
        req.path.includes('/account') ||
        (req.path.includes('/people') && !req.path.includes('/people/update'))
    ) {
        return next();
    }
    ...
    if (platformId === 0) {
        ...
        account = await db_con('accounts')
            .where({ '3ds_service_token': serviceToken });
    } else {
        req.platform = 'wiiu';

        account = await db_con('accounts')
            .where({ wiiu_service_token: serviceToken });
    }
